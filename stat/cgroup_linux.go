...
func (c *cgroupV1) cpuShares() (int64, error) {
	data, err := utils.ReadText(path.Join(c.cgroups["cpu"], "cpu.shares"))
	if err != nil {
		return 0, err
	}
...
func (c *cgroupV1) cpuQuotaUs() (int64, error) {
	data, err := utils.ReadText((path.Join(c.cgroups["cpu"], "cpu.cfs_quota_us")))
	if err != nil {
		return 0, err
	}
...
func (c *cgroupV1) cpuPeriodUs() (uint64, error) {
	data, err := utils.ReadText(path.Join(c.cgroups["cpu"], "cpu.cfs_period_us"))
	if err != nil {
		return 0, err
	}
...
func (c *cgroupV1) cpus() ([]uint64, error) {
	data, err := utils.ReadText(path.Join(c.cgroups["cpuset"], "cpuset.cpus"))
	if err != nil {
		return nil, err
	}
...
func (c *cgroupV1) usageAllCpus() (uint64, error) {
	data, err := utils.ReadText(path.Join(c.cgroups["cpuacct"], "cpuacct.usage"))
	if err != nil {
		return 0, err
	}
...
func (c *cgroupV2) cpuShares() (int64, error) {
	data, err := utils.ReadText(path.Join(c.cgroups["cpu"], "cpu.shares"))
	if err != nil {
		return 0, err
	}
...
func (c *cgroupV2) cpuQuotaUs() (int64, error) {
	data, err := utils.ReadText(path.Join(cgroupDir, "cpu.cfs_quota_us"))
	if err != nil {
		return 0, err
	}
...
func (c *cgroupV2) cpuPeriodUs() (uint64, error) {
	data, err := utils.ReadText(path.Join(cgroupDir, "cpu.cfs_period_us"))
	if err != nil {
		return 0, err
	}
...
func (c *cgroupV2) cpus() ([]uint64, error) {
	data, err := utils.ReadText(cpusetFile)
	if err != nil {
		return nil, err
	}
...
func currentCgroupV1() (cgroup, error) {
	...
	lines, err := utils.ReadTextLines(cgroupFile, utils.WithoutBlank())
	if err != nil {
		return nil, err
	}
...
func currentCgroupV2() (cgroup, error) {
	lines, err := utils.ReadTextLines(cpuStatFile, utils.WithoutBlank())
	if err != nil {
		return nil, err
	}
...
func parseUints(val string) ([]uint64, error) {
	...
	ints := make(map[uint64]utils.PlaceholderType)
